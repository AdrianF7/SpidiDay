(function () {
  ...
  async function detail(date) {
    ...
    if(d.dayNote)root.querySelector('.story-detail')?.insertAdjacentHTML('beforeend',`<section class="story-day-note-card" data-history-note="${date}"><div class="story-day-note-head"><p class="overline">Lo que pasó ese día</p>${canEditDayNote(d)?`<button type="button" class="story-note-edit" data-edit-day-note="${date}">Editar</button>`:''}</div><p class="story-day-note-text">${esc(d.dayNote)}</p></section>`);
  }
  ...
  async function init() {
    ...
    document.addEventListener('click',async e=>{const edit=e.target.closest('[data-edit-day-note]');if(edit){const date=edit.dataset.editDayNote,record=await db.get('dailyHistory',date);if(!canEditDayNote(record)){await detail(date);window.MareaApp?.toast('El plazo de edición ya terminó');return}const card=edit.closest('[data-history-note]');card.innerHTML=`<div class="story-day-note-head"><p class="overline">Editar lo que pasó ese día</p></div><textarea class="story-note-edit-input" maxlength="1200" rows="5">${esc(record.dayNote)}</textarea><div class="story-note-edit-actions"><button type="button" class="secondary" data-cancel-day-note="${date}">Cancelar</button><button type="button" class="primary" data-update-day-note="${date}">Guardar cambios</button></div><p class="day-note-status" role="status" aria-live="polite"></p>`;card.querySelector('textarea')?.focus();return}const cancel=e.target.closest('[data-cancel-day-note]');if(cancel){await detail(cancel.dataset.cancelDayNote);return}const update=e.target.closest('[data-update-day-note]');if(!update)return;const date=update.dataset.updateDayNote,input=update.closest('[data-history-note]')?.querySelector('textarea');if(!input?.value.trim()){window.MareaApp?.toast('La nota no puede quedar vacía');return}update.disabled=true;update.textContent='Guardando…';const record=await saveDayNote(date,input.value,true);if(!canEditDayNote(record)&&record.dayNote!==input.value.trim()){window.MareaApp?.toast('El plazo de edición ya terminó')}else{window.MareaApp?.toast('Nota actualizada')}await detail(date)});
    document.addEventListener('click',e=>{if(e.target.closest('[data-view]'))evaluateClosures(true)});
    document.addEventListener('marea:data-refreshed',async()=>{await syncClosedDayExpenses();await renderTodayClosure();if($('#view-historia')?.classList.contains('active'))await timeline();await evaluateClosures(false)});
  ...
  window.SpidiDayClosure={DAILY_CUTOFF_HOUR,DAILY_CUTOFF_MINUTE,getNow,setNowProvider,getLocalDate,getLocalDayStart,getLocalCutoff,currentState,buildSnapshot,finalizeDay,evaluateClosures,syncClosedDayExpenses,canEditDayNote,saveDayNote,completeLegacyMemory,timeline,detail};
